...
**Verification Note:** This report was generated by directly inspecting the source code and running filesystem checks on September 2, 2026. Storage-file findings may differ across development environments due to .gitignore exclusions — always re-verify on your own machine before relying on file-existence claims.
...
- **Batch Transaction Files (Excel):** Uploaded files contain 15-digit internal General Ledger (GL) account numbers (`111613120722698` and `111613134119657`) under the `Debit Account` column (ingested into database table `BKASH_TRANSACTIONS` as column `source_account_no`).
- **Result:** In `app/Filament/Pages/Dashboard.php`, `calculateBalance(string $accountNumber): float` (lines 399–416) filters debits strictly by exact account string: `BkashTransaction::where('source_account_no', $accountNumber)...`. When 15-digit account transactions are processed against 13-digit initial balance keys, the debits are not matched to the configured initial balances, leaving the dashboard balance card unchanged (displaying the static initial balance rather than reflecting net deductions).
...
### B. Transaction File Layer (`storage/` & Uploads Audit)
Filesystem checks were executed directly against the local storage directory:

**Command 1: `git ls-files storage/ | grep -i xls`**
```
(No files returned — storage directory contents are excluded from git tracking via .gitignore)
```

**Command 2: `find storage -iname "*.xlsx" -o -iname "*.xls" 2>/dev/null`**
```
storage/app/public/Bkash_Files/BEFTN_JANATA_BANK_2026_07_28_2Sloty.xlsx (415,744 bytes)
storage/app/public/Bkash_Files/JANATA_BANK_2026_07_28_1Sloty.xlsx (56,320 bytes)
storage/app/public/Bkash_Files/RTGS_JANATA_BANK_2026_07_28_2Sloty.xlsx (64,512 bytes)
```
*(Note: These production sample files are physically present on this environment's disk in `storage/app/public/Bkash_Files/` but untracked by git due to .gitignore rules. No fictitious root-level sample files exist in the repository.)*
...
The actual balance aggregation is implemented in `app/Filament/Pages/Dashboard.php` (lines 399–416):
```php
